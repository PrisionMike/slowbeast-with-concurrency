...
class Configuration:
    ...
    def draw_immediate_conflicts(e1: TSEState, e2: TSEState) -> None:
        ...
        assert (
            e1.caused_by == e2.caused_by
        ), "Can't have immediate conflict without a common parent"
...
def lock_check(i1: Instruction, i2: Instruction) -> bool:
    return (
        Type(i1) == Call
        and i1.called_function() == "pthread_mutex_lock"
        and Type(i2) == Call
        and i2.called_function() == "pthread_mutex_unlock"
        and i1.operand(0) == i2.operand(0)
    )
...
def data_race_check(i1: Instruction, i2: Instruction) -> bool:
    return (
        Type(i1) == Load
        and Type(i2) == Store
        and i1.pointer_operand() == i2.pointer_operand()
    )
